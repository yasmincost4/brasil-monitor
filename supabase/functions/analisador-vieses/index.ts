...
const MODELO = "claude-sonnet-4-5"; // ajuste pro modelo disponível na sua conta
  ...
  try {
    ...
    for (const h of pendentes ?? []) {
      ...
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: MODELO, max_tokens: 900, system: SISTEMA, messages: [{ role: "user", content: material }] }),
      });
